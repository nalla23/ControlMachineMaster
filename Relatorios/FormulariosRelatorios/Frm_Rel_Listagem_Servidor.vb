...
Public Class Frm_Rel_Listagem_Servidor
    Private Sub Frm_Rel_Listagem_Servidor_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        buscarelatorio()
    End Sub
    Sub buscarelatorio()
        Try
            ...
            Using cn = New SqlConnection(strcon)
                cn.Open()
                Dim sql = "SELECT ID_SERVIDOR, NOME, IP,
                            CASE WHEN S.ativo=1 THEN 'Sim' ELSE 'Não' END AS Ativo
                            FROM SERVIDORES S "
                Using dap = New SqlDataAdapter(sql, cn)
                    Using dt = New DataTable()
                        ...
                        ReportViewer1.LocalReport.ReportPath = caminho_relatorio & ":\ControlMachine\Relatorios\RDLC\Rel_Listagem_Servidor.rdlc"
